...
def update_setting(key, value):
    try:
        ...
        logger.error(f"update_setting error: {e}")
...
def create_user(user_id, name, username):
    try:
        ...
        conn.execute(
            'INSERT INTO users (user_id, name, username, balance) VALUES (?, ?, ?, 0)',
...
# ================= ADEXIUM API (BOT API) ================= #
def get_cached_ad(user_id):
    """ক্যাশ থেকে অ্যাড নিন"""
    if user_id in AD_CACHE:
        data, timestamp = AD_CACHE[user_id]
        if time.time() - timestamp < CACHE_TIME:
            logger.info(f"📌 Using cached ad for {user_id}")
            return data
        else:
            del AD_CACHE[user_id]
    return None

def cache_ad(user_id, ad_data):
    """ক্যাশে অ্যাড সংরক্ষণ করুন"""
    AD_CACHE[user_id] = (ad_data, time.time())
    logger.info(f"💾 Ad cached for {user_id}")

def get_fallback_ad():
    """ফলব্যাক অ্যাড (যখন API কাজ করে না)"""
    return {
        "clickUrl": "https://t.me/onlysazzat",
        "image": "https://img.icons8.com/clouds/200/000000/ad.png",
...
def get_adexium_ad(user_id, name):
    """
    Adexium BOT API থেকে অ্যাড ফেচ করুন
    - ক্যাশ চেক
    - রেট্রাই লজিক (৩ বার চেষ্টা)
    - ফলব্যাক অ্যাড
    """
    
    # 1️⃣ ক্যাশ চেক
    cached = get_cached_ad(user_id)
    if cached:
        return cached
    
    wid = get_setting('adexium_wid')
    if not wid:
        logger.error("❌ No WID found")
    ...
    logger.info(f"📤 Fetching ad for user {user_id}")
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if response.status_code == 200:
                ...
                if result and 'clickUrl' in result:
                    ad_data = {
                        "clickUrl": result.get('clickUrl', 'https://t.me'),
                        "image": result.get('image'),
                        "text": result.get('text', 'Sponsored Ad'),
                        "buttonText": result.get('buttonText', 'Go!')
                    }
                    
                    # ক্যাশে সংরক্ষণ
                    cache_ad(user_id, ad_data)
                    logger.info(f"✅ Ad fetched for {user_id}")
                    return ad_data
                else:
                    logger.warning(f"⚠️ Invalid response: {result}")
            
            # 4️⃣ নো অ্যাড
            elif response.status_code == 404:
                logger.warning("⚠️ No ads available")
                return get_fallback_ad()
    ...
    # 7️⃣ সব চেষ্টা ব্যর্থ - ফলব্যাক
    logger.info("📌 Using fallback ad")
    return get_fallback_ad()

# ================= FLASK SERVER ================= #
@app.route('/click/<token>')
def track_click(token):
    """অ্যাড ক্লিক ট্র্যাক করুন"""
    try:
        conn = get_db()
        if conn:
            conn.execute(
...
"""
        ...
        logger.info(f"✅ Start command for {user_id}")
...
def task_command(message):
    try:
        user_id = message.from_user.id
        logger.info(f"🎯 Task requested by {user_id}")
        ...
        logger.info(f"⚙️ Settings: reward={reward}, wait={wait_time}, server={server_url}")
        
        # 5️⃣ অ্যাড ফেচ (ক্যাশ + রেট্রাই সহ)
        ...
        if conn:
            ...
            conn.commit()
            conn.close()
            logger.info(f"✅ Task saved: {token}")
        else:
            bot.send_message(user_id, "❌ ডেটাবেস সমস্যা!")
            return
        
        # 8️⃣ ট্র্যাকিং লিংক
        track_link = f"{server_url}/click/{token}"
        logger.info(f"🔗 Track link: {track_link}")
...
"""
        ...
        if ad_image and ad_image.startswith('http'):
            try:
                ...
                logger.info(f"✅ Task sent with photo to {user_id}")
            except Exception as e:
        ...
        bot.send_message(message.chat.id, f"❌ সমস্যা হয়েছে! আবার চেষ্টা করুন।")
...
def verify_callback(call):
    try:
        ...
        logger.info(f"🔍 Verify: {token} by {user_id}")
        ...
        if task['is_visited'] == 0:
            conn.close()
            bot.answer_callback_query(call.id, "❌ প্রথমে লিংকে ক্লিক করুন!", show_alert=True)
            return
...
def set_reward(message):
    if message.from_user.id in ADMIN_IDS:
        try:
            ...
            update_setting('per_task_reward', str(value))
...
def set_min(message):
    if message.from_user.id in ADMIN_IDS:
        try:
            value = float(message.text.split()[1])
            update_setting('min_withdraw', str(value))
...
def set_time(message):
    if message.from_user.id in ADMIN_IDS:
        try:
            ...
            update_setting('min_wait_seconds', str(value))
...
def set_wid(message):
    if message.from_user.id in ADMIN_IDS:
        try:
            ...
            update_setting('adexium_wid', value)
...
def test_command(message):
    if message.from_user.id in ADMIN_IDS:
        bot.send_message(message.chat.id, "✅ Bot is working! Server is running.")
...
def clear_cache_command(message):
    if message.from_user.id in ADMIN_IDS:
        global AD_CACHE
        AD_CACHE.clear()
        bot.send_message(message.chat.id, "✅ Cache cleared!")
