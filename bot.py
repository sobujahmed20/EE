...
def get_adexium_ad(user_id, name):
    ...
    logger.info(f"📤 Fetching ad for user {user_id} with WID: {wid}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"✅ Ad fetched: {result}")
            ...
            if result and 'clickUrl' in result:
                ...
                logger.warning(f"⚠️ Invalid ad format: {result}")
        else:
...
def track_click(token):
    logger.info(f"🔗 Click tracking: {token}")
    try:
        conn = get_db()
        if conn:
            conn.execute(
...
def verify_callback(call):
    try:
        ...
        logger.info(f"🔍 Verify called: {token} by {user_id}")
...
def test_command(message):
    """টেস্ট কমান্ড - বট কাজ করছে কিনা চেক করুন"""
    if message.from_user.id in ADMIN_IDS:
